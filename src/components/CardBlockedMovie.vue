<template>
  <v-card position="relative">
    ...
    <div style="position: absolute; top: 0; height: 500px; width: 100%">
      <div class="h-100" @mouseenter="onHover = true" @mouseleave="onHover = false">
        <div class="h-100 w-100" v-show="onHover"
             style="background: rgba(31,31,31,0.7); cursor: pointer; display: flex; justify-content: center; align-items: center;"
             v-on:click="onClick">
          <v-btn class="text-h4" icon="mdi-lock-open-variant" color="#1F1F1F" size="x-large"></v-btn>
        </div>
      </div>
    </div>
  </v-card>
...
export default {
  ...
  methods: {
    onClick() {
      this.$emit('onClickParent', this.movie)
    },
  }
}
</script>
